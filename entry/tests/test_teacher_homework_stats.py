...
class TeacherHomeworkStatsTests(TestCase):
    ...
    def test_teacher_homework_stats_pages_load_for_all_periods_and_use_due_date_label(self) -> None:
        ...
        week_option = next(option for option in week_response.context["period_options"] if option["key"] == "week")
        self.assertEqual(week_option["label"], "周度")
        ...
        self.assertIn("应完成", week_titles)
        self.assertIn("已完成", week_titles)
        self.assertIn("按时完成", week_titles)
        self.assertIn("正确率", week_titles)
        self.assertIn("教师评价", week_titles)
        self.assertNotIn("未完成", week_titles)
        self.assertNotIn("延迟完成", week_titles)
        self.assertNotIn("未设置截止日期", week_titles)
    ...
    def test_anchor_date_2026_05_04_counts_due_date_into_week_month_and_quarter(self) -> None:
        ...
        self.assertEqual(week_response.context["week_range_display_text"], "5月4日 至 5月10日")
        self.assertContains(week_response, "上一周")
        self.assertContains(week_response, "下一周")
        self.assertContains(week_response, "5月4日 至 5月10日")
        self.assertContains(week_response, "anchor_date=2026-04-27")
        self.assertContains(week_response, "anchor_date=2026-05-11")
